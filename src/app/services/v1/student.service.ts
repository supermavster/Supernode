...
export class StudentService {
  ...
  all = async () => {
    const data = await this.studentRepository.all();
    // Check if Exist
  ...
  index = async (request: IComplements.ID) => {
    ...
    const data = await this.studentRepository.index({id});
    // Check if Exist
    if (typeof data === 'undefined' || !data) {
      return {
        status: false,
  ...
  remove = async (request: IComplements.ID) => {
    const id: number = request.id;
    const data = await this.studentRepository.destroy({id});
    return {
      status: true,
      data,
  ...
  update = async (key: IComplements.ID, request: IComplements.CRUDImage) => {
    ...
    request.slug = sha1(request.name.toString().trim()).toString();
    const data = await this.studentRepository.update(request, {id});
    if (typeof data === 'undefined' || !data) {
      return {
        status: false,
        message: lang.STACK.CRUD.ERROR.MAKE
