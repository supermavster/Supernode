...
export class ContentService {
  ...
  all = async () => {
    const data = await this.contentRepository.all();
    // Check if Exist
  ...
  index = async (request: IComplements.ID) => {
    ...
    const data = await this.contentRepository.index({id});
    // Check if Exist
    if (typeof data === 'undefined' || !data) {
      return {
        status: false,
  ...
  remove = async (request: IComplements.ID) => {
    const id: number = request.id;
    const data = await this.contentRepository.destroy({id});
    return {
      status: true,
      data,
  ...
  create = async (request: IComplements.CRUD) => {
    const data = await this.contentRepository.create(request);
    if (typeof data === 'undefined' || !data) {
      return {
        status: false,
  ...
  update = async (key: IComplements.ID, request: IComplements.CRUD) => {
    ...
    const data = await this.contentRepository.update(request, {id});
